module AtomicLti
  class Examples
    ...
    def run
      if AtomicLti::Definitions.deep_link_launch?(@lti_token)
    ...
    def add_to_settings(settings)
      ...
      if @is_deep_link
        settings[:deep_link_settings] = @lti_token[AtomicLti::Definitions::DEEP_LINKING_CLAIM]
      end
    ...
    # AtomicLti::Services::Score class
    def scores_example(line_item, names_and_roles)
      return unless names_and_roles.present?

      score_service = AtomicLti::Services::Score.new(@application_instance, @lti_token)
      score_service.id = line_item["id"]
      names_and_roles["members"].map do |name_role|
        in_role = !(name_role["roles"] &
          [AtomicLti::Definitions::STUDENT_SCOPE, AtomicLti::Definitions::LEARNER_SCOPE]).empty?
    ...
    # AtomicLti::Services::Results class
    ...
    # AtomicLti::Services::NamesAndRoles class
    def names_and_roles_example
      names_and_roles_service = AtomicLti::Services::NamesAndRoles.new(@application_instance, @lti_token)
