module AtomicLti
  class LtiUser
    ...
    def user
      ...
      if user.blank?
        ...
        if user.legacy_lti_user_id.blank?
          user.legacy_lti_user_id = @lti_token[AtomicLti::Definitions::LTI11_LEGACY_USER_ID_CLAIM]
        end
        _update_roles(user)
        user.save!
      end
    ...
    def _migrate_lti_user
      user = User.find_by(lti_user_id: @lti_token[AtomicLti::Definitions::LTI11_LEGACY_USER_ID_CLAIM])
      if user
    ...
    def _add_roles(user)
      roles = @lti_token.dig(AtomicLti::Definitions::ROLES_CLAIM)
      roles.each do |role|
